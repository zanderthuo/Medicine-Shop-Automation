...
$r=mysqli_fetch_assoc($t);
	...
	//insert a bill
 	$qry="INSERT INTO `bill` (`bill_id`, `shop_id`, `patient_id`, `date`, `total`, `paid`, `processed`) VALUES (NULL, '".$shop_id."', '".$r['patient_id']."', '".date("Y-m-d")."', '', '', '0');";
  	...
  	$qry="Select p.product_id,p.quantity, SUM(s.quantity) as available from pres_meds p,stock s where p.pres_id='$pres_id' and s.shop_id='$shop_id' and s.product_id=p.product_id  group by p.product_id";
	...
	if($cnt==0){
			...
			$qry="update prescriptions set bought='2' where pres_id='$pres_id'";
			mysqli_query($conn,$qry);

			header("Location:../prescriptions.php?msg=0");
